...
int main()
{
    vector<int>v1= {1,2,3,4,5,6,7,9,0,1,2,3,44,54,44,76,67,6,67,6767,676,76,767,67,67,6};
    v.clear();
    v.resize(20);
    cout<<v.size()<<endl;
    for(auto u:v) cout<<u<<" ";
    ...
    v.erase(v.begin());
    cout<<*v.begin()<<endl;
    sort(v.begin(),v.end()); // uniq Value index return
    int size=unique(v.begin(),v.end())-v.begin();
    cout<<size<<endl;
    for(auto u:v) cout<<u<<" ";
    for(int i=0; i<size; i++) cout<<v[i]<<" ";
    vector<int>::iterator it=max_element(v.begin(),v.begin()+8);
    cout<<*it<<endl;
    vector<int>::iterator it=min_element(v.begin(),v.begin()+8);
    cout<<*it<<endl;
///2D vector
    vector<vector<int> >v;
    vector<int > a1= {2,3,4};
    vector<int > a2= {2,3,4,8,9,0};
    v.push_back(a1);
    v.push_back(a2);
    for(auto u:v)
    {
